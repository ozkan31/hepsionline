...
export default async function OdemeBasarisizPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const belongsToUser =
    !order || order.customerEmail === currentUser.email || order.customerPhone === currentUser.phone;

  const orderNo = order ? `#${order.orderNo ?? order.id}` : null;
  const maskedEmail = order ? maskEmail(order.customerEmail) : maskEmail(currentUser.email);
        ...
        <div className="max-w-3xl w-full text-center">
          ...
          <p className="mt-6 text-[15px] leading-7 text-slate-600">
            {belongsToUser && orderNo ? (
              ...
              <>Odeme tamamlanmadi. Lutfen odemeyi tekrar deneyin.</>
            )}
            <br />
